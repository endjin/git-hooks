...
Function Scan-ConfigFiles([System.IO.FileInfo[]] $files)
{
    ...
    foreach($file in $files)
    {
		Try
		{
            ...
            {     
				if ($workingCopy.configuration.appSettings.HasChildNodes -and $workingCopy.configuration.appSettings.ChildNodes.Count > 1)
				{
			...
			if ($workingCopy.configuration.runtime.assemblyBinding -ne $null -and $workingCopy.configuration.runtime.assemblyBinding.ChildNodes.Count > 1){
				if ($workingCopy.configuration.runtime.assemblyBinding.HasChildNodes){
